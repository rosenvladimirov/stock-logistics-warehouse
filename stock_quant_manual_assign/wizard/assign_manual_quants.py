...
class AssignManualQuants(models.TransientModel):
    ...
    owner_id = fields.Many2one(comodel_name='res.partner', string='Owner', readonly=True, groups="stock.group_tracking_owner",)
    use_package_id = fields.Many2one('stock.quant.package', string="Choice target package")
    used_package_ids = fields.One2many("stock.quant.package", compute="_compute_used_package_ids")

    @api.depends('move_id')
    def _compute_used_package_ids(self):
        for record in self:
            packages = record.move_id.move_line_ids.mapped("result_package_id")
            #_logger.info("PACKAGE %s:%s" % (packages, record.move_id))
            record.used_package_ids = packages

    def _assign_quants(self, ml):
        if self.use_package_id and not ml.result_package_id:
            ml.result_package_id = self.use_package_id
    ...
    def _assign_quant_line(self):
        ...
        if float_compare(self.qty, 0.0, precision_digits=precision_digits) > 0:
            available_quantity = quant._get_available_quantity(
                ...
                package_id=self.package_id, owner_id=self.assign_wizard.owner_id and self.assign_wizard.owner_id or self.owner_id,
            )
            if float_compare(
            ...
            move._update_reserved_quantity(
                ...
                owner_id=self.assign_wizard.owner_id and self.assign_wizard.owner_id or self.owner_id, strict=True
            )
